...
// Dùng process.cwd() để paths luôn relative tới thư mục chạy lệnh (repo root)
const SESSION_PATH = path.join(process.cwd(), "zalo_session");
const IMAGES_PATH = path.join(process.cwd(), "images");
...
class Bot extends EventEmitter {
  constructor(config) {
    ...
    // Che do server: luon headless, emit screenshot QR de phuc vu qua HTTP
  ...
  async start() {
    try {
      ...
      if (!hasSession) {
        ...
        // Server mode: chup screenshot dinh ky de phuc vu qua /qr
        let qrInterval = null;
        if (this.serverMode) {
